...
bool isVisible(uint64_t mesh)
{
	...
	float fLastRenderTimeOnScreen = driver.read<float>(mesh + 0x334);
...
std::string readwtf(uintptr_t Address, void* Buffer, SIZE_T Size)
{
	...
	driver.read_buffer(Address, Buffer, Size);
...
FTransform Boneindex(DWORD_PTR mesh, int index) {
	DWORD_PTR bonesarray = driver.read<DWORD_PTR>(mesh + 0x590);
	if (bonesarray == NULL) {
		bonesarray = driver.read<DWORD_PTR>(mesh + 0x590 + 0x10);
	}
	return driver.read<FTransform>(bonesarray + (index * 0x60));
}
...
std::uintptr_t find_signature(const char* sig, const char* mask)
{
	...
	{
		driver.read_buffer(sdk::module_base + i * 0x100000, data, 0x100000);
		...
		for (std::uintptr_t j = 0; j < 0x100000u; ++j)
		{
				...
				}(data + j, (std::uint8_t*)sig, mask))
			{
				...
				driver.read_buffer(result + 3, &rel, sizeof(std::uint32_t));
...
static std::string ReadGetNameFromFName(int key)
{
	...
	uint64_t NamePoolChunk = driver.read<uint64_t>(base_addr + 0xCC282C0 + (8 * ChunkOffset) + 16) + (unsigned int)(4 * NameOffset); //((ChunkOffset + 2) * 8) ERROR_NAME_SIZE_EXCEEDED
...
CamewaDescwipsion UndetectedCamera(__int64 a1)
{
	...
	v1 = driver.read<__int64>((DWORD_PTR)LocalPlayers + 0xC8);
	__int64 v9 = driver.read<__int64>(v1 + 8);


		VirtualCamera.FieldOfView = 80.f / (float)((float)driver.read<double>(v9 + 0x690) / 1.19f);
	...
	v6 = driver.read<__int64>((DWORD_PTR)LocalPlayers + 0x70);
...
void fn_cache() {
	...
	//	if (!offset_uworld) offset_uworld = find_signature(skCrypt("\x48\x89\x05\x00\x00\x00\x00\x48\x8B\x4B\x78"), skCrypt("xxx????xxxx")) - sdk::module_base;
		...
		Uworld = driver.read<uint64_t>(base_addr + 0xcbf3538);
		//	std::cout << "\nUworld: " << Uworld;
		uint64_t GameInstance = driver.read<uint64_t>(Uworld + offsets::game_instance);
			//std::cout << "\nGameInstance: " << GameInstance;
		LocalPlayers = driver.read<uint64_t>(driver.read<uint64_t>(GameInstance + offsets::local_players));
		//	std::cout << "\nLocalPlayers: " << LocalPlayers;
		PlayerController = driver.read<uint64_t>(LocalPlayers + offsets::player_controller);
		//std::cout << "\nLocalPlayers: " << LocalPlayers;
		CameraManager = driver.read<uint64_t>(PlayerController + offsets::player_camera_manager);
		//	std::cout << "\nCameraManager: " << CameraManager;
		LocalPawn = driver.read<uint64_t>(PlayerController + offsets::acknowledged_pawn);
		//std::cout << "\nLocalPawn: " << LocalPawn;
		LocalRootcomp = driver.read<uint64_t>(LocalPawn + offsets::root_component);
		//	std::cout << "\nLocalRootcomp: " << LocalRootcomp;
		LocalRelativeLocation = driver.read<Vector3>(LocalRootcomp + offsets::relative_loc);

		DWORD itemCount = driver.read<DWORD>(Uworld + (offsets::levels + sizeof(PVOID)));

		//	auto t = LI_FN(GetTickCount).get()();
		for (int i = 0; i < itemCount; i++) {
			uintptr_t ItemLevels = driver.read<uintptr_t>(Uworld + offsets::levels);
			if (!ItemLevels) return;

			uintptr_t ItemLevel = driver.read<uintptr_t>(ItemLevels + (i * sizeof(uintptr_t)));
			if (!ItemLevel) return;

			for (int i = 0; i < driver.read<DWORD>(ItemLevel + (offsets::aactors + sizeof(PVOID))); ++i) {

				uintptr_t ItemsPawns = driver.read<uintptr_t>(ItemLevel + offsets::aactors);
				uintptr_t CurrentItemPawn = driver.read<uintptr_t>(ItemsPawns + (i * sizeof(uintptr_t)));
				int currentitemid = driver.read<int>(CurrentItemPawn + offsets::item_definition);
				auto NameCurrentActor = GetNameFromFName(currentitemid);
			//	std::cout << "\ncurrentitemid: " << currentitemid;
			//	std::cout << "\nGetNameFromFName: " << NameCurrentActor;
				uint64_t mesh = driver.read<uint64_t>(CurrentItemPawn + offsets::mesh);

				uint64_t LocalPlayerState = driver.read<uint64_t>(LocalPawn + offsets::player_state);
				uint64_t PlayerState = driver.read<uint64_t>(CurrentItemPawn + offsets::player_state);
				uint64_t rootcomponent = driver.read<uint64_t>(CurrentItemPawn + offsets::root_component);
				uint32_t LocalTeamId = driver.read<uint32_t>(LocalPlayerState + offsets::team_index);
				uint32_t TeamId = driver.read<uint32_t>(PlayerState + offsets::team_index);

				uintptr_t ItemRootComponent = driver.read<uintptr_t>(CurrentItemPawn + offsets::root_component);
		...
		{
			auto localcurwep = driver.read<uint64_t>(LocalPawn + offsets::CurrWeapon);
			auto definition = driver.read<uint64_t>(localcurwep + offsets::weapondata);
			auto wtf = driver.read<uint64_t>(definition + offsets::item_definition);
			//std::cout << "niigger";
...
void actor_loop(forceinline::dx_renderer* renderer) {
	...
	for (auto entity : entityListCopy)
	{
		...
		Vector3 Relativelocation = driver.read<Vector3>(entity.root + offsets::relative_loc); //this invalid
...
#pragma endregion
			...
			if (distance <= settings::MaxDistance && IsInScreen(vNeckOut))
			{
				...
				if (settings::threed_box && LocalPawn && PlayerController) {
					FBoxSphereBounds EntityBounds = driver.read<FBoxSphereBounds>(entity.mesh + 0x738); //USkinnedMeshComponent	CachedWorldSpaceBounds	0x738	FBoxSphereBounds
				...
				{
					auto curWep = driver.read<uint64_t>(entity.Actor + offsets::CurrWeapon);
					auto itemDef = driver.read<uint64_t>(curWep + offsets::weapondata); // -> UFortWeaponItemDefinition
					auto itemName = driver.read<uintptr_t>(itemDef + offsets::display_name);//all corect, checked not sure about second ones dont remove tier shit

					if (!itemName) continue;
					if (!itemDef) continue;
					auto bIsReloadingWeapon = driver.read<bool>(curWep + offsets::is_reloading);
					uint32_t StringLength = driver.read<uint32_t>(itemName + 0x38);

					if (StringLength <= 32) {
						auto itemName2 = driver.read<uintptr_t>(itemName + 0x30);

						auto tier = driver.read<BYTE>(itemDef + offsets::tier);
						...
						driver.read_buffer((ULONG64)driver.read<PVOID>(itemName + 0x30), OutString, StringLength * sizeof(wchar_t));
	...
	if (settings::chestesp || settings::ammoboxesp || settings::vehicle || settings::lootesp) {
		for (auto entity : itemListCopy)
		{
			...
			if (IsInScreen(ChestPosition) && LocalPawn && PlayerController) {
				...
				bool bAlreadySearched = driver.read<bool>(entity.Actor + offsets::balreadysearched);
				std::string null = skCrypt("").decrypt();
				...
				if (settings::lootesp && strstr(entity.name.c_str(), skCrypt("FortPickupAthena"))) {
					...
					if (ItemDist < 100.0f) {
						...
						auto definition = driver.read<uint64_t>(entity.Actor + offsets::primary_pick_up_item_entry + offsets::item_definition); //0x308
						auto wtf = driver.read<uint64_t>(definition + offsets::item_definition);

						auto names = GetNameFromFName(wtf);

						tier = driver.read<BYTE>(definition + offsets::tier);


						auto DisplayName = driver.read<uint64_t>(definition + offsets::display_name); // DisplayName (FText)
						auto WeaponLength = driver.read<uint32_t>(DisplayName + 0x38); // FText -> Length
						wchar_t* WeaponName = new wchar_t[uint64_t(WeaponLength) + 1];

						driver.read_buffer((ULONG64)driver.read<PVOID>(DisplayName + 0x30), WeaponName, WeaponLength * sizeof(wchar_t));
	...
	if (closestPawn != 0 && LocalPawn && PlayerController) {
		if (settings::aimbot && (GetAsyncKeyState_Spoofed(thekey) || shouldaimController))
		{
			...
			auto is_dying = (driver.read<char>(closestPawn + offsets::is_dying) >> 2) & 1;
			auto is_knocked = (driver.read<char>(closestPawn + offsets::is_dbno) >> 4) & 1;
			...
			if (show_menu) {
				if (menu_system.Window(skCrypt("KingCU.cc"), pos, FVector2D(400, 400), show_menu)) {
					...
					if (buenotab == 2)
					{
						...
						menu_system.Text(" DavCU820 For the Name");
...
void init()
{
	std::cout << skCrypt("\n\n [!] KingCU.cc Fortnite Cheat\n");

	Sleep(2000);
	system("start https://github.com/Buck3ts41");
	std::cout << skCrypt("\n\n [-] Connecting to driver\n");


	    driver.init();
		mouse_interface();
		CreateThread_Spoofed(0, 0, (LPTHREAD_START_ROUTINE)controllerSupport, 0, 0, 0);
		CreateThread_Spoofed(0, 0, (LPTHREAD_START_ROUTINE)keySystem, 0, 0, 0);
		CreateThread_Spoofed(0, 0, (LPTHREAD_START_ROUTINE)random_aim, 0, 0, 0);
		std::cout << " [x] Estabilizing Connecction with the driver.\n";
		if (!driver.init())
		{
			printf(" [x] Initialization or communication test failed.\nPlease make sure driver is loaded.\n");
		    Sleep(1500);
		    exit(69);
		}
		...
		auto pid_str = sGetProcessId(L"FortniteClient-Win64-Shipping.exe");
		driver.attach(uint32_t(pid_str));
		...
		sdk::game_wnd = FindWindowA_Spoofed(0, "Fortnite  ");
